...
-- -----------------------------------------------------
-- Table `spve`.`periodo_venta_producto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS spve.periodo_venta_producto ;

CREATE SEQUENCE spve.periodo_venta_producto_seq;

CREATE TABLE IF NOT EXISTS spve.periodo_venta_producto (
  id_periodo_venta_producto INT NOT NULL DEFAULT NEXTVAL ('spve.periodo_venta_producto_seq'),
  nombre_periodo_venta VARCHAR(45) NOT NULL,
  activo_periodo_venta SMALLINT NOT NULL,
  PRIMARY KEY (id_periodo_venta_producto))
;
...
CREATE TABLE IF NOT EXISTS spve.producto (
  ...
  descripcion_empaque VARCHAR(45),
  cantidad_disponible DECIMAL(10,0) DEFAULT 0 NOT NULL,
  ...
  id_periodo_venta_producto INT,
  PRIMARY KEY (id_producto),
  CONSTRAINT fk_producto_periodo_venta_producto1
    FOREIGN KEY (id_periodo_venta_producto)
    REFERENCES spve.periodo_venta_producto (id_periodo_venta_producto)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
;
...
-- -----------------------------------------------------
-- Table `spve`.`precio_producto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS spve.precio_producto ;

CREATE SEQUENCE spve.precio_producto_seq;

CREATE TABLE IF NOT EXISTS spve.precio_producto (
  id_precio_producto INT NOT NULL DEFAULT NEXTVAL ('spve.precio_producto_seq'),
  fecha_registro_precio DATE NOT NULL,
  margen_ganancia DECIMAL(10,0),
  porcentaje_impuesto_producto REAL,
  impuesto_producto DECIMAL(10,0),
  precio_venta_publico DECIMAL(10,0) NOT NULL,
  base_imponible REAL,
  producto_exento SMALLINT DEFAULT 0 NOT NULL,
  activo_precio_producto SMALLINT NOT NULL DEFAULT 1,
  id_producto INT NOT NULL,
